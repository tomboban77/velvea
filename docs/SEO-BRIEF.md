...
| V-15 ownership / analytics | **Done (code)** | Vercel Analytics + Speed Insights already present. `GOOGLE_SITE_VERIFICATION` / `BING_SITE_VERIFICATION` emit the ownership meta tags when set. GA4 (`NEXT_PUBLIC_GA_MEASUREMENT_ID`) loads with Consent Mode v2 all-denied; the storefront consent banner grants `analytics_storage` on accept and remembers the choice; ad signals stay denied; privacy policy gains GA paragraphs only when GA is configured. **Live 2026-09-21:** GA4 property "Velvea" (stream G-G4WZGMMQH5) receiving data, URL-param redaction on (session_id, token, q); Search Console `https://www.velvea.ca/` verified via HTML tag (`GOOGLE_SITE_VERIFICATION` set in Vercel) with sitemap.xml submitted; Bing Webmaster Tools imported from Search Console (no Bing token needed). Owner account: giftsvelvea@gmail.com. Microsoft Clarity not added. |
...
| L-02 / L-07 delivery hub + city pages | **Done (code) 2026-09-21** | `/delivery` hub and 11 city pages (`src/lib/cities.ts`, `src/app/[locale]/delivery`). Fees, cutoffs and same-day/next-day/courier status are resolved at request time from the delivery zones via a representative FSA, so admin zone edits flow through with no code change. Each city has its own bilingual intro and neighbourhood list (no templated doorway copy). Service + BreadcrumbList + FAQPage JSON-LD. Hub-only cities (Hamilton, Durham, north York Region, Guelph, Orangeville) are listed but get no page until they earn one. Owner decision 2026-09-21: same-day core unchanged plus Burlington, Georgetown, Bolton to be moved into the Brampton/Oakville/Milton zone in admin. Hamilton next-day: undecided. |
| V-11 llms.txt | **Done (code) 2026-09-21** | `/llms.txt` route (`src/app/llms.txt/route.ts`): plain-text site summary, key pages, city pages, browse links. AI crawlers remain allowed by robots.txt. |
| Search vocabulary | **Done 2026-09-21** | Owner approved. "hampers", "gift boxes" and "online gift shop" added to page titles/descriptions (`messages/*.json` → `meta.*`), the hero lede and CTA, the /baskets heading and lede, and the root default title. "Gift baskets" remains the lead phrase everywhere. FR uses "paniers garnis", "coffrets", "boutique de cadeaux en ligne". Hamilton stays courier shipping (owner decision). |
| L-04 Business Profile | **Done (owner) 2026-09-22** | Profile created under giftsvelvea@gmail.com and submitted for verification (Google processing, up to 5 days, no promised date). Service-area setup with the address hidden: primary category "Gift shop", service areas Guelph, Milton and 10 other Ontario areas, hours 8:00 AM–6:00 PM seven days, phone (431) 726-1706, website https://www.velvea.ca/. Matches the Organization-with-areaServed shape in `src/lib/seo.ts` (no LocalBusiness node). Site hours aligned to Mon–Sun 8am–6pm in `DEFAULT_SETTINGS`; the owner keeps the studio address published on the site while hiding it on the profile, and keeps the Manitoba-area-code phone as the single consistent number. Outstanding on the profile itself: description, secondary category, attributes, photos, products. Add the Maps URL to `settings.social` (→ `sameAs`) once public. |
| V-16 Merchant feed, V-14 performance baseline, V-12 content plan | **Not started** | Merchant feed waits on the owner creating a Merchant Center account; content plan needs published guides. |
...
|---|---|
| ~~Which Search Console/Bing accounts exist?~~ Answered 2026-09-21: both created under giftsvelvea@gmail.com, verified, sitemap submitted. Merchant Center: none. Business Profile: created 2026-09-22 under the same account, service-area listing with a hidden address, verification in progress (see the L-04 row); the earlier "not applicable" reading is superseded — free studio pickup is the qualifying in-person contact. | Merchant Center only once purchase-ready |
...
| ~~Staffed customer visits/pickup?~~ Answered 2026-09-21: no premises customers can visit, online only. Owner to reconcile the FAQ "pickup from our Mississauga studio" line and the PICKUP delivery zone with that. | LocalBusiness closed as N/A |
...
**Answered 2026-09-22:** profile created and submitted for verification; service-area setup, address hidden, primary category "Gift shop". Eligibility rests on free pickup from the Mississauga studio. If pickup is genuinely staffed during the posted hours, revisit the storefront/hybrid option after verification clears — a service-area-only listing does not compete for the Mississauga local pack. Profile fields still to complete and the `sameAs` link are tracked in the status table.
